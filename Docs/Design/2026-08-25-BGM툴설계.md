...
- **진행 길이가 4 이하면 화음은 마디 경계에서만 바뀐다. 5 이상이면 반 마디에서도 바뀐다.**
  온마디 셀(한 마디를 한 음으로 채우는 패턴)은 마디 가운데에서 바뀐 화음을 칠 수가 없어서다 (2026-09-18).
...
| --- | --- | --- |
| 실행 파일 | `SoundTool/external/fluidsynth/bin/fluidsynth.exe` | `--fluidsynth` |
| 사운드폰트 | `SoundTool/external/soundfont/GeneralUser-GS.sf2` | `--soundfont` |

상수는 **패키지 위치 기준 상대 경로**로 잡는다 (`Path(__file__).parents[3] / "external" / …`).
...
SoundTool/
  ...
  external/                 # git 제외. fluidsynth/ · soundfont/
  .venv/               # git 제외
```
...
1. **FluidSynth 2.6.0**
   ...
   풀어서 `SoundTool/external/fluidsynth/` 에 통째로 둔다 → `SoundTool/external/fluidsynth/bin/fluidsynth.exe`.
...
2. **GeneralUser GS 2.0.3**
   ...
   `documentation/LICENSE.txt` 를 받아 `SoundTool/external/soundfont/` 에 둔다.
3. **Python** : `py -3.14 -m venv SoundTool/.venv` 뒤 `numpy` · `mido` · `pytest` 를 넣는다.
   `mido` 1.3.3 은 순수 Python 이라 3.14 에서 깔린다 (조사 문서 2-4).
4. **`.gitignore`** : `SoundTool/.gitignore` 에 `external/` 은 이미 있다. **`.venv/` 와 `Out/` 를 더한다.**
...
스튜디오 저장소의 공수기록에 적힌 눈금을 따랐다. 「설계가 문장까지 정한 문서 구현은 ×0.5」 를 코드에도 적용하되,
...
| 2 | FluidSynth·사운드폰트를 **키트 안 `external/`** 에 둔다 (git 제외) | PATH 설치 · 받는 스크립트 | 툴 폴더가 그 자체로 완결돼야 한다. GeneralUser GS 는 "직접 링크 걸지 말라"는 부탁이 있다 |
