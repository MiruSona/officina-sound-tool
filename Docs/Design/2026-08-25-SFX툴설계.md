...
| 상수 | 값 | 덮어쓰기 |
| --- | --- | --- |
| `RFXGEN_PATH` | `<패키지 루트>/../../external/rfxgen.exe` | `--rfxgen <경로>` 또는 환경변수 `SOUNDTOOL_RFXGEN` |
...
```
SoundTool/
  external/rfxgen.exe            (git 제외, 이미 있음)
  src/soundtool/
    __init__.py
    __main__.py             CLI
    config.py               상수
...
스튜디오 저장소 규칙의 눈금대로 **갈래 3개 × 30분 = 90분의 합치는 공수**를 뒷정리에 이미 담았다.
...
- **rfxgen.exe 를 키트에 어떻게 얹나.** 지금은 `SoundTool/external/` 에 두고 git 에서 뺐다.
